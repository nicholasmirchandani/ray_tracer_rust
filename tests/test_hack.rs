...
macro_rules! expect_eq {
    ($a:expr, $b:expr) => {
        ...
        unsafe { crate::test_hack::TEST_PASSED = crate::test_hack::TEST_PASSED && ($a == $b); } 
    };
}
...
macro_rules! expect_float_eq {
    ($a:expr, $b:expr) => {
        ...
        if !passed {
            println!("Test Failed: {} not equal (per floating point comparison) to {}.  [{}:{}]", $a, $b, module_path!(), line!());
        }
        unsafe { crate::test_hack::TEST_PASSED = crate::test_hack::TEST_PASSED && passed; } 
    };
}
